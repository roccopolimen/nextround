// FYI when i did /config/mongoConnection absolute pathing it errored so ya'll can fix that
...
// DB SETUP
    ...
    const cycleCollection = await db.collection('cycles');
    const mediaCollection = await db.collection('media');

// FUNCTIONS TO MAKE OBJECTS 
    const makeUser = (id, email, name) =>{
        return {
            _id: id,
            email: email,
            name: name,
    ...
    const makeMedia = (id, posterId, jobCycle, postDate, content) => {
        return {
            _id: id,
            posterId: posterId,
            jobCycle: jobCycle,
    ...
    const makeCycle = (id, startDate, endDate) => {
        return {
            _id: id,
            startDate: startDate,
            endDate: endDate,
    ...
    const makeApplication = (id, company, position, location, salary, cardColor, progress, jobPostUrl, description) => {
        return {
            _id: id,
            company: company,
            position: position,
    ...
    const makeEvent = (id, status, title, date, location) => {
        return {
            _id: id,
            status: status,
            title: title,
    ...
    const makeContact = (id, name, pronouns, location, phone, email) => {
        return {
            _id: id,
            name: name,
            pronouns: pronouns,
    ...
    const addCycleToUser = (user, cycleId) => {
        user.cycles.push(cycleId);
    };

    const addApplicationToCycle = (cycle, application) => {
        cycle.applications.push(application);
    };

    const addNoteToApplication = (application, note) => {
        application.notes.push(note);
    };

    const addEventToApplication = (application, event) => {
        application.events.push(event);
    };

    const addContactToApplication = (application, contact) => {
...
// CREATE USERS
    const michael = makeUser(new ObjectId('624e16475be3fb3026fa8bb4'), 'mkarsen@stevens.edu', 'Michael Karsen');
    const rocco = makeUser(new ObjectId('624e169643afefcc06801ea2'), 'rpolimen@stevens.edu', 'Rocco Polimeni');
    const marco = makeUser(new ObjectId('624e16aa16662f0fa5a22f78'), 'mpolimen@stevens.edu', 'Marco Polimeni');
    const brian = makeUser(new ObjectId('624e16ad52210043ef57890a'), 'bwormser@stevens.edu', 'Brian Wormser');
    const grace = makeUser(new ObjectId('624e16b16bf9399b0f8d3687'), 'gmattern@stevens.edu', 'Grace Mattern');

// CREATE CYCLES
    const michaelFall = makeCycle(new ObjectId('624e956c4c06f2509b940b1d'), new Date('September 22, 2021'), new Date('December 20, 2021'));
    const roccoFall = makeCycle(new ObjectId('624e36a9c78fc2142856b173'), new Date('September 22, 2021'), new Date('December 20, 2021'));
    const roccoSpring = makeCycle(new ObjectId('624e181fc086e2c5081110ca'), new Date('March 20, 2021'), new Date('June 19, 2021'));
    const marcoSummer = makeCycle(new ObjectId('624e36b2ae8647ff7668ab04'), new Date('June 20, 2021'), new Date('September 21, 2021'));
    const marcoFall = makeCycle(new ObjectId('624e36ae80109e5a1cce8516'), new Date('September 22, 2021'), new Date('December 20, 2021'));
    const brianSpring = makeCycle(new ObjectId('624e36cf865d56f8a9a07c71'), new Date('March 20, 2021'), new Date('June 19, 2021'));
    const graceWinter = makeCycle(new ObjectId('624e199cec15debf1e33b54a'), new Date('December 21, 2021'), new Date('March 19, 2022'));
...
// CREATE EVENTS
    ...
    // Google events
    const googleEvent1 = makeEvent(new ObjectId('624e26fb85375bb9375ae04a'), false, 'Apply', new Date('September 24, 2021'), 'Online');
    const googleEvent2 = makeEvent(new ObjectId('624e324e7c4f596651ddba11'), false, 'Phone Interview', new Date('September 27, 2021'), 'Phone');
    const googleEvent3 = makeEvent(new ObjectId('624e3253799a20c356c9cef7'), false, 'In Person Interview', new Date('October 10, 2021'), 'In Person');
    ...
    // Apple events
    const appleEvent1 = makeEvent(new ObjectId('624e3256e0b7bde5c2634843'), true, 'Apply', new Date('September 28, 2021'), 'Online');
    const appleEvent2 = makeEvent(new ObjectId('624e325a74555dc07f5cc1c4'), false, 'Phone Interview', new Date('October 1, 2021'), 'Phone');
    const appleEvent3 = makeEvent(new ObjectId('624e3984d3c0dff0ef801fd2'), false, 'Technical Interview', new Date('October 5, 2021'), 'Online');
    const appleEvent4 = makeEvent(new ObjectId('624e325d6ffcd86beaf6b99d'), false, 'In Person Interview', new Date('October 12, 2021'), 'In Person');
    ...
    // Meta events
    const metaEvent1 = makeEvent(new ObjectId('624e325f1c62e84fab6b390f'), true, 'Apply', new Date('September 20, 2021'), 'Online');
    const metaEvent2 = makeEvent(new ObjectId('624e3262676ebcef2a4a4fdb'), true, 'Phone Interview', new Date('September 23, 2021'), 'Phone');
    const metaEvent3 = makeEvent(new ObjectId('624e32660ed84646f83ce09c'), true, 'Technical Interview', new Date('September 30, 2021'), 'Online');
    addEventToApplication(meta, metaEvent1);
    addEventToApplication(meta, metaEvent2);
    addEventToApplication(meta, metaEvent3);

    // Amazon events
    const amazonEvent1 = makeEvent(new ObjectId('624e326959f20a0932331466'), true, 'Apply', new Date('June 25, 2021'), 'Online');
    const amazonEvent2 = makeEvent(new ObjectId('624e326c28190989e6555ec7'), true, 'Phone Interview', new Date('June 30, 2021'), 'Phone');
    const amazonEvent3 = makeEvent(new ObjectId('624e326f124149fbc3ddf59c'), true, 'Technical Interview', new Date('July 5, 2021'), 'Online');
    const amazonEvent4 = makeEvent(new ObjectId('624e39c6ecb3a48dd0b60f5f'), true, 'In Person Interview', new Date('July 14, 2021'), 'In Person');
    ...
    // Microsoft events
    const microsoftEvent1 = makeEvent(new ObjectId('624e3272afc8bd8aa7fd09cd'), true, 'Apply', new Date('October 5, 2021'), 'Online');
    const microsoftEvent2 = makeEvent(new ObjectId('624e32750d376ab4ebda7a13'), false, 'Phone Interview', new Date('October 9, 2021'), 'Phone');
    const microsoftEvent3 = makeEvent(new ObjectId('624e327857414a900bed8f15'), false, 'Technical Interview', new Date('October 14, 2021'), 'Online');
    ...
    // HRT events
    const hrtEvent1 = makeEvent(new ObjectId('624e327b2428725014db7ddf'), false, 'Apply', new Date('March 26, 2021'), 'Online');
    const hrtEvent2 = makeEvent(new ObjectId('624e327e9f51bd07e8480c04'), false, 'Phone Interview', new Date('March 30, 2021'), 'Phone');
    const hrtEvent3 = makeEvent(new ObjectId('624e3281115427fee6306918'), false, 'Technical Interview', new Date('April 3, 2021'), 'Online');
    const hrtEvent4 = makeEvent(new ObjectId('624e3a48f0cce62a3e2bfe47'), false, 'In Person Interview', new Date('April 10, 2021'), 'In Person');
    const hrtEvent5 = makeEvent(new ObjectId('624e3a781b4ca271f3c6d2da'), false, 'Peer Interview', new Date('April 12, 2021'), 'In Person');
    ...
    // JPMorgan events
    const jpmEvent1 = makeEvent(new ObjectId('624e3283622c59a07e43bf45'), true, 'Apply', new Date('August 10, 2021'), 'Online');
    const jpmEvent2 = makeEvent(new ObjectId('624e32870f7e38bb3e667112'), false, 'Phone Interview', new Date('August 13, 2021'), 'Phone');
    const jpmEvent3 = makeEvent(new ObjectId('624e32890b7ff5f93863306b'), false, 'Technical Interview', new Date('August 16, 2021'), 'Online');
    ...
    const boaEvent1 = makeEvent(new ObjectId('624e328c359f4c73586ca3b5'), true, 'Apply', new Date('January 4, 2022'), 'Online');
    const boaEvent2 = makeEvent(new ObjectId('624e328f94869178a8c1b9c3'), true, 'Phone Interview', new Date('January 7, 2022'), 'Phone');
    const boaEvent3 = makeEvent(new ObjectId('624e329277b5952a2121a3e9'), true, 'Technical Interview', new Date('January 14, 2022'), 'Online');
    addEventToApplication(boa, boaEvent1);
    addEventToApplication(boa, boaEvent2);
    addEventToApplication(boa, boaEvent3);

    // Netflix events
    const netflixEvent1 = makeEvent(new ObjectId('624e3295586193609ba847cf'), true, 'Apply', new Date('November 5, 2021'), 'Online');
    const netflixEvent2 = makeEvent(new ObjectId('624e32978eeeb12e21965496'), true, 'Phone Interview', new Date('November 8, 2021'), 'Phone');
    const netflixEvent3 = makeEvent(new ObjectId('624e329b14c7adca2034a372'), true, 'Technical Interview', new Date('November 13, 2021'), 'Online');
    const netflixEvent4 = makeEvent(new ObjectId('624e3aa0f02ff0a59c9a8522'), true, 'In Person Interview', new Date('November 20, 2021'), 'In Person');
    ...
    // Airbnb events
    const airbnbEvent1 = makeEvent(new ObjectId('624e32a1fe8cf8822b6a95c8'), true, 'Apply', new Date('May 18, 2021'), 'Online');
    const airbnbEvent2 = makeEvent(new ObjectId('624e32a83f205bfc1b4ef3d6'), false, 'Phone Interview', new Date('May 21, 2021'), 'Phone');
    const airbnbEvent3 = makeEvent(new ObjectId('624e32ab220f1d07a5e65a48'), false, 'In Person Interview', new Date('May 30, 2021'), 'In Person');
    ...
    // Tesla events
    const teslaEvent1 = makeEvent(new ObjectId('624e32ae8aac5c8f246d6cc1'), true, 'Apply', new Date('July 7, 2021'), 'Online');
    const teslaEvent2 = makeEvent(new ObjectId('624e32b167b752c087805532'), true, 'Phone Interview', new Date('July 11, 2021'), 'Phone');
    const teslaEvent3 = makeEvent(new ObjectId('624e32b44c22cee92f738e94'), true, 'Technical Interview', new Date('July 16, 2021'), 'Online');
    const teslaEvent4 = makeEvent(new ObjectId('624e3aaad146f4e35ad6512c'), true, 'In Person Interview', new Date('July 22, 2021'), 'In Person');
    ...
    // Oracle events
    const oracleEvent1 = makeEvent(new ObjectId('624e32b7e3c2a93080344db7'), true, 'Apply', new Date('Febuary 3, 2022'), 'Online');
    const oracleEvent2 = makeEvent(new ObjectId('624e32ba43263a9c18551f75'), true, 'Phone Interview', new Date('Febuary 6, 2022'), 'Phone');
    const oracleEvent3 = makeEvent(new ObjectId('624e32bc730509575dfdd6ed'), false, 'In Person Interview', new Date('Febuary 12, 2022'), 'In Person');
    ...
    // Salesforce events
    const salesforceEvent1 = makeEvent(new ObjectId('624e32bfea2deb8b92ed6885'), true, 'Apply', new Date('March 1, 2022'), 'Online');
    const salesforceEvent2 = makeEvent(new ObjectId('624e32c297f7b96fea3c978f'), true, 'Phone Interview', new Date('March 4, 2022'), 'Phone');
    const salesforceEvent3 = makeEvent(new ObjectId('624e32c62b99b9eb7564e7b7'), true, 'Technical Interview', new Date('March 10, 2022'), 'Online');
    const salesforceEvent4 = makeEvent(new ObjectId('624e3ba8cfa94fd794556d3a'), true, 'In Person Interview', new Date('March 16, 2022'), 'In Person');
    ...
    // Bloomberg events
    const bloombergEvent1 = makeEvent(new ObjectId('624e32c9b33b9492bcc2eb33'), true, 'Apply', new Date('September 22, 2021'), 'Online');
    const bloombergEvent2 = makeEvent(new ObjectId('624e32d4ec4550d308eb7c4e'), false, 'Phone Interview', new Date('September 25, 2021'), 'Phone');
    const bloombergEvent3 = makeEvent(new ObjectId('624e32d05cbbdb20b2867768'), false, 'Technical Interview', new Date('September 29, 2021'), 'Online');
    const bloombergEvent4 = makeEvent(new ObjectId('624e3bd1b606773034ea1142'), false, 'In Person Interview', new Date('October 4, 2021'), 'In Person');
...
// CREATE CONTACTS
    ...
    // Google contacts
    const googleContact1 = makeContact(new ObjectId('624e6c2caa6fa6d3b4e69e72'), 'Peter	Jackson', 'he/him', 'location', '(987) 011-3615', 'pjackson@google.net');
    addContactToApplication(google, googleContact1);

    // Apple contacts
    const appleContact1 = makeContact(new ObjectId('624e6c32731515cd2758d4ef'), 'Victor	Mills', 'he/him', 'location', '(297) 334-0517', 'vmills@apple.net');
    addContactToApplication(apple, appleContact1);

    // Meta contacts
    const metaContact1 = makeContact(new ObjectId('624e6c351b0d313ec7849d23'), 'Matt Miller', 'he/him', 'location', '(985) 714-1627', 'mmiller@meta.net');
    addContactToApplication(meta, metaContact1);

    // Amazon contacts
    const amazonContact1 = makeContact(new ObjectId('624e6c38f4786e42217ed8ad'), 'Peter Burgess', 'he/him', 'location', '(233) 275-9568', 'pburgess@amazon.net');
    addContactToApplication(amazon, amazonContact1);

    // Microsoft contacts
    const microsoftContact1 = makeContact(new ObjectId('624e6c3c2ae281da0ba6c12a'), 'Dylan Bond', 'he/him', 'location', '(447) 660-9680', 'dbond@microsoft.com');
    addContactToApplication(microsoft, microsoftContact1);

    // HRT contacts
    const hrtContact1 = makeContact(new ObjectId('624e6c40f697ee33f2bf5686'), 'Megan Payne', 'she/her', 'location', '(159) 735-9568', 'mpayne@hrt.net');
    addContactToApplication(hrt, hrtContact1);

    // JPMorgan contacts
    const jpmContact1 = makeContact(new ObjectId('624e6c43bda4f0b6a52636ff'), 'Emily Sutherland', 'she/her', 'location', '(259) 609-5747', 'esuther@jpmorgan.com');
    addContactToApplication(jpmorgan, jpmContact1);

    // Bank of America contacts
    const boaContact1 = makeContact(new ObjectId('624e6c466c8d16579ab273dc'), 'Connor Paige', 'he/him', 'location', '(521) 476-4591', 'cpaige@bofa.com');
    addContactToApplication(boa, boaContact1);

    // Netflix contacts
    const netflixContact1 = makeContact(new ObjectId('624e6c49ae8e2e1911e19a65'), 'Mary	Dyer', 'she/her', 'location', '(106) 634-8114', 'mdyer@netflix.com');
    addContactToApplication(netflix, netflixContact1);

    // Airbnb contacts
    const airbnbContact1 = makeContact(new ObjectId('624e6c4c0e49b28d57248fe0'), 'Owen Cameron', 'he/him', 'location', '(543) 295-4084', 'ocameron@airbnb.net');
    addContactToApplication(airbnb, airbnbContact1);

    // Tesla contacts
    const teslaContact1 = makeContact(new ObjectId('624e6c507d3e7468c8bc7655'), 'Austin	Bower', 'he/him', 'location', '(926) 412-3368', 'abower@tesla.net');
    addContactToApplication(tesla, teslaContact1);

    // Oracle contacts
    const oracleContact1 = makeContact(new ObjectId('624e6c52893bb7397e3e1768'), 'Joanne Allan', 'she/her', 'location', '(967) 824-6026', 'jallan@oracle.net');
    addContactToApplication(oracle, oracleContact1);

    // Salesforce contacts
    const salesforceContact1 = makeContact(new ObjectId('624e6c562fa1e9a4691ed62a'), 'Joshua Lewis', 'he/him', 'location', '(207) 965-8964', 'jlewis@salesforce.com');
    addContactToApplication(salesforce, salesforceContact1);

    // Bloomberg contacts
    const bloombergContact1 = makeContact(new ObjectId('624e6c59b9a243097f967952'), 'Joseph Glover', 'he/him', 'location', '(976) 813-9168', 'jglover@bloomberg.net');
...
//CREATE MEDIA
    ...
    // Post 1
    const michaelPost = makeMedia(new ObjectId('624e7432b9bd8ffe79422272'), michael._id, michaelFall._id, new Date('December 19, 2021'), 'I absolutely killt it you already know!');

    // Post 2
    const roccoPost = makeMedia(new ObjectId('624e743cd787b4472fbe1dc6'), rocco._id, roccoSpring._id, new Date('June 18, 2021'), 'I\'m a straight clown no cap, gonna have to hit myself with a chop on foenem ( 。ヘ °)');

    // Post 3
    const marcoPost = makeMedia(new ObjectId('624e743fce646315d8c99d4b'), marco._id, marcoFall._id, new Date('December 19, 2021'), 'Hopefully this job is able to purchase my dad that beach house he always wanted :D');
    
    // Post 4
    const brianPost = makeMedia(new ObjectId('624e7446030bb2f1a8a9d3b6'), brian._id, brianSpring._id, new Date('June 17, 2021'), 'Brian more like brain');

    // Post 5
    const gracePost = makeMedia(new ObjectId('624e744a074e859c77121e2e'), grace._id, graceWinter._id, new Date('March 19, 2022'), 'I GOT THE JOB WOOT WOOT');
